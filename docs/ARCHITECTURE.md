...
7. **`core/sync`**: Bidirectional delta op synchronization between local and Google Drive storage following strict version control semantics ("push only local new, pull only remotely new"):
   - **Granular Push ("Push only local new")**: Only operations recorded in the `offlineQueue` (local new operations) trigger uploads. Uploads are strictly mapped to the specific modified files (e.g. `space.manifest.update` pushes only `manifest.json`; `object.create/update` pushes only `objects/${target}.json`; `graph.node.*` pushes only `graph/nodes.json`; `graph.edge.*` pushes only `graph/edges.json`). No bulk uploading of untouched files.
   - **Granular Pull ("Pull only remotely new")**: Only remote delta operations not in the local `OperationLog` (`!localOpIds.has(op.operation_id)`) are pulled. If no new remote operations exist, zero entity files are downloaded. When new ops exist, only the files modified by those specific new operations are downloaded (e.g., `space.manifest.update` pulls only `manifest.json` and dynamically updates the active space manifest, loaded space memory, and user registry).
   - **Space Manifest & Settings in Version Control**: Space name, settings, description, icon, and categories updates emit `space.manifest.update` operations with Lamport timestamping, allowing collaborative settings updates to sync deterministically across devices and users without bulk transfers.
