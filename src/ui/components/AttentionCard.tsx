/**
 * Attention Card — Interactive Active Layer Alert with 1-click micro-actions
 */
...
export const AttentionCard: React.FC<{ alert: LADActiveAlert }> = ({ alert }) => {
  const { updateObject, dismissAlert, snoozeAlert } = useLAD();
  const { t } = useI18n();
        ...
        {alert.type === 'stale_balance' && isUpdatingBalance && (
          ...
          >
            ...
            <motion.button
              ...
              className="px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold whitespace-nowrap shadow-sm"
            >
              Save $
            </motion.button>
            <button
